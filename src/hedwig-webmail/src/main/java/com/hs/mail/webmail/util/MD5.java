...
public final class MD5 {
	 ...
	 * <p>
	 * Following the the MD5 standard specification, the result is returned
	 * least significant byte first, however, many applications use the most
	 * significant byte first (more conventional).
	 * 
	 * @param msg
	 *            the <tt>byte[]</tt> to be digested.
	 * 
	...
	private static final int round1(int a, int b, int c, int d, int k, int s,
			int i, int[] x) {
		return b + rot((a + F(b, c, d) + x[k] + T[i - 1]), s);
	}

	private static final int round2(int a, int b, int c, int d, int k, int s,
			int i, int[] x) {
		return b + rot((a + G(b, c, d) + x[k] + T[i - 1]), s);
	}

	private static final int round3(int a, int b, int c, int d, int k, int s,
			int i, int[] x) {
		return b + rot((a + H(b, c, d) + x[k] + T[i - 1]), s);
	}

	private static final int round4(int a, int b, int c, int d, int k, int s,
			int i, int[] x) {
		return b + rot((a + I(b, c, d) + x[k] + T[i - 1]), s);
	}

	private static final int T[] = { 0xd76aa478, 0xe8c7b756, 0x242070db,
	...
	public static void main(String[] args) {
		try {
			if (args == null || args.length == 0) {
				System.out
						.println("Usage: java dtw.webmail.util.MD5 <password>");
			}
