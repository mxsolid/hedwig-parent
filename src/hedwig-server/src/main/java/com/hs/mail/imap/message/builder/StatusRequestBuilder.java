 ...
 *
 */
public class StatusRequestBuilder extends ImapRequestBuilder {
	...
	public ImapRequest createRequest(String tag, String command,
		...
		Status statusAtts = new Status();
		while (!tokens.isEmpty()) {
			Token token = tokens.remove();
			DecoderUtils.decodeStatusAttList(token.value, statusAtts);
		}
		return new StatusRequest(tag, command, mailbox, statusAtts);
	}
