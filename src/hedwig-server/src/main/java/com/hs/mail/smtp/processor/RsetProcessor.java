...
/**
 * Handler for RSET command. Reset reverse-path buffer, forward-path buffer and
 * data buffer.
 * 
 * @author Won Chul Doh
 * @since May 29, 2010
 * 
 */
public class RsetProcessor extends AbstractSmtpProcessor {
	...
	protected void doProcess(SmtpSession session, TcpTransport trans,
		...
		if (st.countTokens() > 0) {
			...
			session.writeResponse("250 2.0.0 Reset state");
		}
	}
