 ...
 *
 */
public class LoginRequestBuilder extends ImapRequestBuilder {

	@Override
	public ImapRequest createRequest(String tag, String command,
			ImapMessage message) {
		LinkedList<Token> tokens = message.getTokens();
		String username = tokens.remove().value;
		String password = tokens.remove().value;
		return new LoginRequest(tag, command, username, password);
	}
