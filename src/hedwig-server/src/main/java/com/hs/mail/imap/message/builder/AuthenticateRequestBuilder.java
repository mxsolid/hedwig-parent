 ...
 *
 */
public class AuthenticateRequestBuilder extends ImapRequestBuilder {

	@Override
	public ImapRequest createRequest(String tag, String command,
			ImapMessage message) {
		LinkedList<Token> tokens = message.getTokens();
		String authType = tokens.remove().value;
		return new AuthenticateRequest(tag, command, authType);
	}
