 ...
 *
 */
public class RenameRequestBuilder extends ImapRequestBuilder {

	@Override
	public ImapRequest createRequest(String tag, String command,
			ImapMessage message) {
		LinkedList<Token> tokens = message.getTokens();
		String oldname = tokens.remove().value;
		String newname = tokens.remove().value;
		return new RenameRequest(tag, command, oldname, newname);
	}
